...
- `auth_mode` (string, optional provider-table key): selects a provider-specific authentication contract. For Kimi Code, the supported route is `auth_mode = "api_key"` (or omit the field), `base_url = "https://api.kimi.com/coding/v1"`, `model = "kimi-for-coding"`, and a Kimi Code API key. Legacy, manually authored `[providers.moonshot] auth_mode = "kimi_oauth"` is read-only compatibility: Codewhale may use an already unexpired Kimi CLI access token, but never auto-enables the import, starts a Kimi login, refreshes the token, or rewrites the CLI credential file. Missing, malformed, and expired imports fail closed with API-key guidance; first-class OAuth requires Codewhale's own vendor-registered client identity and remains tracked in #4417.
